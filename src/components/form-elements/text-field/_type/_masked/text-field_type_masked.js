class MaskedTextInput {
  constructor($maskedInput) {
    $maskedInput.on('keydown', MaskedTextInput.handleMaskedInputKeydown);
    $maskedInput.on('input', MaskedTextInput.handleMaskedInputInput);
  }

  static handleMaskedInputKeydown(event) {
    const { key } = event;
  ...
  static handleMaskedInputInput(event) {
    const { target } = event;
    const { value } = target;
    target.value = MaskedTextInput.fixDate(value);
  }

  static getDatePartsArray(sourceValue) {
  ...
  static fixDate(sourceValue) {
    const dateParts = MaskedTextInput.getDatePartsArray(sourceValue);
...
new MaskedTextInput($maskedInput);
